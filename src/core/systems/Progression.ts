import { EXP_NEEDED_MULTIPLIER, EXP_GAIN_MULTIPLIER } from "../../controllers/GameState.js";

export class Progression {
  getRequiredExp(level: number) { // Current level
    const multiplier = 10 * EXP_NEEDED_MULTIPLIER;
  ...
  addExp(amount: number, currentExp: number, currentLevel: number) {
    ...
    let statPoints = 0;
    currentExp += (amount * EXP_GAIN_MULTIPLIER);
